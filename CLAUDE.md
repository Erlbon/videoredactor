...
## Domain notes
- External tools, not bundled: ffmpeg (thumbnails, remux) and MKVToolNix (`mkvpropedit`, `mkvmerge`, `mkvextract`), found via override, bundled `tools/`, PATH, or their default install folder (MKVToolNix's installer doesn't add itself to PATH). The app prompts to install missing ones and must not block launch. Every call goes through `redactor_common.core.subprocess_utils.run_tool()` (no console window, `stdin=DEVNULL`, UTF-8 output, timeout).
